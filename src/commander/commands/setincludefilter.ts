/**
 * Setincludefilter command - replace the include-recursive filter with specified IDs.
...
export const setincludefilterCommand: CommandDefinition = {
    name: 'setincludefilter',
    description: 'Set include-recursive filter to specified node IDs',
    aliases: ['sif'],
    positionals: [
        {
            name: 'nodeIds',
            description: 'Node IDs to include recursively (uses cursor if omitted)',
            required: false,
            complete: (partial) => {
    ...
    handler: (args) => {
        ...
        setFilter({ ...filter, includeRecursive: nodeIds });
        output.success(`includeRecursive set: ${nodeIds.join(', ')}`);
    },
};
